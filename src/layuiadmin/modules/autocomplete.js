layui.define(['jquery', 'laytpl', 'layer'], function (e) {
    ...
    job.prototype.pullData = function () {
        ...
        var params=_config.params;
        params[_config.key]=_config.filter;
        (!_config.cache && _config.ajax[_self.index] != undefined) && _config.ajax[_self.index].abort(), _config.ajax[_self.index] = $.ajax({
            ...
            success: function (resp) {
                return "ok" != eval('resp.' + _config.response.code) ? layer.msg(eval('resp.' + _config.response.data)) : _config.data[_self.index] = eval('resp.' + _config.response.data), _self.renderData(_config.data[_self.index])
            },
            error: function () {
                hint.error("请求失败")
            },
